...
namespace LivingStoryteller
{
    ...
    public static class StorytellerPersonaDatabase
    {
        ...
        static StorytellerPersonaDatabase()
        {
            ...
            {
                Log.Message($"[LivingStoryteller] Loaded persona def: {def.defName} | Storyteller: {def.storytellerDefName} | VoiceId: {def.voiceId} | VoiceProviders: {(def.voiceProviders != null ? string.Join(", ", def.voiceProviders.Select(vp => $"{vp.name}:{vp.voice}")) : "none")} | Accent: {def.accent} | PersonaText length: {def.personaText?.Length ?? 0}");
            }
        }
