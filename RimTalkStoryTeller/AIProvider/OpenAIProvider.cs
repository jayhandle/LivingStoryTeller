    ...
    internal class OpenAIProvider : IAIProvider
    {
        ...
        public async Task<TTSResponseData> GetTTSResponse(string json)
        {
            ...
            using (var resp = await httpClient.PostAsync(url, content))
            {
                ...
                byte[] bArry = Encoding.UTF8.GetBytes(responseBody); //needs to be tested with actual TTS response, may need to be base64 decoded or something else depending on the API
